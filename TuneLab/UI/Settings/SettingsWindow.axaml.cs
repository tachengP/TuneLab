...
internal partial class SettingsWindow : Window
{
    public SettingsWindow()
    {
        ...
        }
        {
            ...
            {
                var checkBox = new GUI.Components.CheckBox();
                checkBox.Bind(Settings.ParaSyncMode, false, s);
                panel.AddDock(checkBox, Dock.Right);
            }
            {
                var name = new TextBlock() { Text = "Para Sync Mode".Tr(this) + ": ", VerticalAlignment = Avalonia.Layout.VerticalAlignment.Center };
                panel.AddDock(name);
            }
            listView.Content.Children.Add(panel);
        }
